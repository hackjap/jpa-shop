...
### 상품 목록 조회
- thymeleaf에서 ? 는 null을 무시해줌
- JPA에서 엔티티를 의존성없이 최대한 순수하게 설계하는것이 가장 중요 !
  - 엔티티를 유연하게 사용( 유지보수의 장점 )
  - 화면에 맞는 폼 객체나 , DTO를 사용해야함
- API를 설계할때 절대로 엔티티를 반환하면 안됨
  - 보안 문제
  - API의 스펙이 변경됨
  - 템플릿 엔진에서는 사용가능 
  - 가장 권장하는 방식은 DTO로 반환하는 것이 좋다.
- 실무에서는 컨틀롤러에서도 setter를 없애는 것이 좋음
...
- 변경 감지 기능 사용
- 병합(merge) 사용 
  - 준영속 상태를 영속 상태로 변경할 떄 사용하는 기능
  - !하지만 병합시 값이 없으면 null로 업데이트 할 위험도 있다...
  - ! 그래서 결국 복잡한 실무에서는 변경감지를 사용해야 한다.
  
### 정리
 - 엔티티를 변경할 떄는 항상 변경 감지를 사용(가장 좋은 방법 )
 - 컨트롤러에서 어설프게 엔티티를 
...
# API 개발 
- api 패키지를 만들어 분리하자 ( 추후 예외처리 등, 공통처리를 하기 위해 )

### 회원등록
- 파라미터에 @Valid 시 , @Empty 로 된 부분을 적용
- 하지만 엔티티에서 검증 로직이 들어가면 해당 엔티티를 사용하는 API들의 스펙 자체가 모두 변경되어 주의를 해야한다.
- 따라서 API의 요청 스펙에 따른 별도의 DTO를 만들어 사용해야 한다.
- 기능에 따른 별도의 dto를 생성하여 파라미터로 받아서 사용 ( 엔티티와 API 스펙을 명확히 분리 .)
- 항상 반복하지만 외부에 "엔티티를 노출"하지 않는 것이 가장 중요 !

### 회원수정 
- post 대신 put을 사용
- Response ,Request 클래스를 이용
- 커맨드와 쿼리를 분리 ( 데이터 변경시, 변경감지 방식 사용)

## 회원조회
- 엔티티 노출시 , @JsonIgnore -> 최악 , api가 엔티티에 종속됨
- 추가로 Result 클래스로 컬렉션을 감싸서 향후 필요한 필드를 추가할 수 있다
- 항상 api 경로를 주의하자 ( 앞 뒤 / 확인)
...
### 지연 로딩과 조회 성능 최적화(엄청중요!!)
- 지금부터 실무에서 JPA를 사용하려면 100% 이해해야함 !!
- 엔티티를 직접 노출할 때는 양방향 연관관계가 걸린 곳은 꼭 한 곳을 @JsonIgnore 처리 안그러면 무한루프
- 하지만 Hibernate5Module을 사용해서 엔티티 노출하는 것은 좋지 않다.
- 실무에서 문제의 성능문제의 90% n+1  , 따라서 fetch join을 완벽히 이해해야한다.
...
### 컬렉션 조회 최적화
- 일대다 관계를 조회해야함 
- distinct 키워드 객체의 아이디가 같은 중복된 것들을 제거해줌 
  - 하지만, 일대다 fetjoin 에서는 페이징이 불가능하다.
- 컬렉션 페이조인은 한가지만 사용해야 한다.

### 페이징 한계 돌파
- 컬렉션을 페치조인하면 페이징 불가능 
  - 일대다 조인이 발생하므로 데이터를 예측할 수 없이 증가 
  - 일대다에서 일을 기준으로 페이징하는 것이 목적인데, 다를 기준으로 row가 생성
- 하이버네이트는 경고 로그를 남기고 모든 DB를 읽어서 메모리에서 페이징을 시도 , 최악의 경우 장애 

 #### 한계 돌파 
  - 페이징 + 컬렉션 엔티티를 함께 조회하려면 ?
    - ToOne 관계는 모두 fetch join
    - 컬렉션은 지연로딩으로 조회
    - 지연 로딩 성능 최적화를 우ㅟ해 hibernate.default_batch_fetch
